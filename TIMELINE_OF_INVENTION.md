...
| 2026-05-19 | — | CF Firewall Block List in intel report — `intel::render_html` adds a **CF Firewall Block List** section: collects THREAT IPs + IPs with existing CF firewall blocks, generates a ready-to-paste CF expression `(ip.src in {IP1 IP2 ...})` with per-IP reason + org + rDNS table. Redacted report shows count only; internal report shows full expression. 269 tests, 14/14 standards. |
| 2026-05-19 | — | `whobelooking tail` — real-time log watcher. Polls file size every `--interval` seconds (default: 30); re-runs full parse→aggregate→rDNS→history→render pipeline on each change. rDNS results cached in-process so subsequent iterations only resolve new IPs. Prints `[HH:MM:SS] refreshed → file.html (N IPs, M threats)` each pass. 269 tests, 14/14 standards. |
| 2026-05-19 | — | `whobelooking install` — systemd self-host story. Writes `~/.config/systemd/user/whobelooking.service` (user-service, `Restart=on-failure`, `ExecStart` uses current binary path) and prints `systemctl --user daemon-reload / enable --now / status` instructions. `--unit-path` override for non-standard setups. 269 tests, 14/14 standards. |
| 2026-05-19 | — | SDVOSB copy consistency — README.md and `/skill` contact section both said "SDVOSB Pending"; updated to "SDVOSB Certified" to match the `sdvosb-certified.svg` badge added in 6af872f. |
| 2026-05-19 | — | Trend/delta view in HTML reports — sidebar panel gains a **Trend** section when actor history is present: shows "N new IPs (M threats) / N returning (M threats)". Derived from `history_total_reports` field already embedded per-IP by `render`. Zero new data collection; hidden when no history exists (fresh install). 269 tests, 14/14 standards. |
| 2026-05-19 | — | Screenshots refreshed — `docs/screenshots/` updated via headless Chromium against live `localhost:8082`; new `report-page.png` added showing rendered HTML report with confidence scores, trend panel, 2949 events / 836 IPs. |
