...
/// Serialize all in-memory Prometheus counters to bincode+zstd on SIGTERM.
/// Called from the OLD process's graceful-shutdown handler — runs before the
/// process exits so the next process can restore exact running totals.
/// Write-then-rename so the file is never half-written.
#[cfg(feature = "serve")]
pub fn save_state(project: &str) {
    let state = crate::web::metrics::ProcessState::capture();
    let encoded = match bincode::serde::encode_to_vec(&state, bincode::config::standard()) {
        Ok(b) => b,
        Err(e) => {
            tracing::error!("state bincode serialize failed: {}", e);
            return;
        }
    };
    let compressed = match zstd::encode_all(encoded.as_slice(), 3) {
        Ok(c) => c,
        Err(e) => {
            tracing::error!("state zstd compress failed: {}", e);
            return;
        }
    };
    ...
    if let Err(e) = std::fs::write(&tmp, &compressed) {
        tracing::error!("state write failed: {}", e);
        return;
    }
    if let Err(e) = std::fs::rename(&tmp, &path) {
        tracing::error!("state rename failed: {}", e);
        return;
    }
    tracing::info!(
        "state snapshot saved ({} bytes compressed) — Gemini Man handoff ready",
        compressed.len()
    );
}

/// Load a prior process's counter snapshot and add its values to the current
/// process's atomics, then delete the file. Called from the NEW process after
/// kill_old() returns — old process has exited so the file is complete.
/// Silent on missing file (cold start). Warns on decode errors (corrupt/stale).
#[cfg(feature = "serve")]
pub fn load_state(project: &str) {
    let path = state_path(project);
    if !path.exists() {
        return;
    }
    let compressed = match std::fs::read(&path) {
        Ok(b) => b,
        Err(e) => {
            tracing::warn!("state read failed: {}", e);
            return;
        }
    };
    ...
    let decoded = match zstd::decode_all(compressed.as_slice()) {
        Ok(b) => b,
        Err(e) => {
            tracing::warn!("state zstd decompress failed: {}", e);
            return;
        }
    };
    let state: crate::web::metrics::ProcessState =
        match bincode::serde::decode_from_slice(&decoded, bincode::config::standard()) {
            Ok((s, _)) => s,
            Err(e) => {
                tracing::warn!("state bincode deserialize failed: {}", e);
                return;
            }
        };
    state.apply();
    tracing::info!(
        "state restored from snapshot (saved_at={}) — counters are continuous",
        state.saved_at
    );
}
...
#[allow(dead_code)] // public utility, kept for ops scripts
